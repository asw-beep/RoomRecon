...
# notebook <sha>: one %%bash cell. %%bash raises on a non-zero exit, so the Kaggle status
...
notebook() {
  python - "$1" "$GIT_URL" <<'EOF'
import json, sys
sha, url = sys.argv[1], sys.argv[2]
src = f"""%%bash
